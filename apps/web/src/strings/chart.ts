...
export const chart = {
  ...
  graph: {
    ...
    toolbar: {
      ...
      fitAll: '全部顯示',
      relayout: '重新排列',
      relayoutTip: '把拖亂的節點、還有自己拉過大小的框，全部放回自動佈局算出來的樣子',
      fitAllTip: '把整張圖縮到看得完。縮放平移過之後畫面不會自己跳回全景，要按這裡',
      boxSelect: '框選',
      boxSelectOn: '✓ 框選',
      boxSelectTipOn:
      ...
      groupRelated: '相關（不影響排程）',
      /** 選了哪一類就要從對應的那個圓點拉，掛在「拉線建立」上滑過去看 */
      newLinkTipScheduling: '排程類有先後：從節點右側的圓點，拉到下一張任務的左側',
      newLinkTipRelated: '相關類沒有先後：從節點下緣的圓點，拉到另一張任務的上緣',
    },
     ...
     * 圓點跟任務節點一樣拖得動，但它不是任務 —— 不能點開、也不能從它拉線。
     *
     * 兩頭都要接得到任務，所以提示也把兩頭一起講出來：
     * 只講「從這裡分出去」的話，看的人還是不知道這個點在等誰。
     */
    junction: {
      fork: '同時開始',
      join: '同時完成',
      forkTip: '同時開始：左邊那張任務一到位，這幾張就從這個時間點一起起跑',
      joinTip: '同時完成：這幾張在這個時間點一起收尾，右邊那張才接得下去',
    },
     ...
     * 最下面那條說明列。
     *
     * 只解釋「看不出來的東西」—— 線上本來就掛著中文短句（完成後開始、相關…），
     * 說明列再列一次同樣的字等於同一件事講兩遍，所以線的那一排整排拿掉了。
     * 留下來的是形狀：框、以及同時開始／同時完成的圓點，那兩種猜不到。
     */
    legend: {
      rowShape: '框與圓點',
      rowIcon: '圖示',
    ...
    help: {
      operation:
        ...
        '・雙擊節點：開啟那張任務\n' +
        '・排程關聯：從節點右側的圓點，拉到下一張任務的左側（種類用上面的「拉線建立」選）\n' +
        '・任務相關：從節點下緣的圓點，拉到另一張任務的上緣 ——\n' +
        '　這幾種沒有先後，所以走上下，不走左右\n' +
        '・點線：刪除那條關聯\n' +
        '・拖節點可以自己排版，同時開始／同時完成的圓點一樣拖得動\n' +
        '・點一下大項目的框，四邊與四角就可以拉，把框改成自己要的大小\n' +
        '・按「重新排列」把位置與大小全部放回自動算出來的樣子\n' +
        '・「框選」開著時左鍵拉出範圍選多張，一起拖曳；右鍵平移畫面\n' +
        '・滾輪縮放，或用左上角的＋／－。縮放平移過之後畫面就不再自己跳回全景，\n' +
        '　要看全部按「全部顯示」',
      ...
      box:
        ...
        '框的大小可以自己拉：點一下框，四邊與四角就會出現把手。\n' +
        '縮不到比裡面的任務還小，免得框把裡面的東西蓋掉；\n' +
        '按「重新排列」就回到自動算出來的大小。',
      ...
      icon: {
        ...
        /*
         * 同時開始／同時完成在畫面上有兩個長相：圖上的圓點、節點上的徽章。
         * 那是同一件事，所以說明只留這一份，一次把兩個長相講完 ——
         * 說明列上不再另外開一項「匯合點」。
         */
        sameStart: {
          label: '⇉ 同時開始',
          text: '跟別的任務同一天開始。排人力時這幾張要一起看，同一天都得到位。\n' +
                '圖上是一個橘色小圓點：它不是兩張任務之間的箭頭，而是一個時間點 ——\n' +
                '一支箭頭進來、多支從這裡分出去。節點上則是「⇉ 同時開始 N」這個徽章。\n' +
                '圓點的左邊一定接得到一張任務，那就是這幾張共同在等的東西；\n' +
                '前面沒有任何任務在等的時候不畫圓點，直接在兩張任務之間拉一條線 ——\n' +
                '一個接不到東西的點比不畫還難懂。\n' +
                '圓點拖得動，但它不是任務，點不開也不能從它拉線。',
        },
        sameFinish: {
          label: '⇥ 同時完成',
          text: '跟別的任務同一天完成。驗收、結案會撞在同一天。\n' +
                '圖上是一個紫色小圓點：多支箭頭進來，在這個時間點收成一支出去。\n' +
                '節點上則是「⇥ 同時完成 N」這個徽章。\n' +
                '圓點的右邊一定接得到一張任務，那就是要等這幾張全部做完的東西；\n' +
                '後面沒有任務接手的時候不畫圓點，直接在兩張任務之間拉一條線。\n' +
                '圓點拖得動，但它不是任務，點不開也不能從它拉線。',
        },
        overlap: {
          label: '⇉ 並行',
      ...
      inquiry: {
        AWAITING: '對外詢問：發出去了，還在等對方回覆，也還沒到期望回覆日。',
        OVERDUE:  '對外詢問：過了期望回覆日還沒回。逾期是查詢時算出來的，不是存下來的狀態。',
        PARTIAL:  '對外詢問：問了好幾個單位，有的回了、有的還沒。',
        REPLIED:  '對外詢問：都回覆了。回覆的單位可能跟提問的單位不同（轉單位）。',
      },
    },
  },
