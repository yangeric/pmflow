...
export const chart = {
  ...
  graph: {
    ...
    help: {
      scheduling: {
        FS: '等待事件完成，才能開始：連結線左邊為順序高（先執行的事件），右邊為順序低（後執行的事件），實線由左向右流向。',
        SS: '等待事件開始，才能開始：連結線左邊為順序高（先執行的事件），右邊為順序低（後執行的事件），實線由左向右流向。',
        FF: '等待事件完成，才能完成：連結線左邊為順序高（先執行的事件），右邊為順序低（後執行的事件），實線由左向右流向。',
        SF: '等待事件開始，才能完成：連結線左邊為順序高（先執行的事件），右邊為順序低（後執行的事件），實線由左向右流向。',
      },
      operation:
        '・點節點：只留亮它的鄰居，其餘淡出。再點一次取消\n' +
        '・雙擊節點：開啟那張事件\n' +
        '・排程關聯：連結線左邊為順序高、右邊為順序低。從順序高節點右側圓點，拉到順序低節點左側圓點（類型用上面的「拉線建立」選）\n' +
